...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 building_facade.py train --dataset=path/to/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 building_facade.py train --dataset=/path/to/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 building_facade.py train --dataset=/path/to/dataset --weights=imagenet

    # Apply color splash to an image
    python3 building_facade.py splash --weights=/path/to/weights/file.h5 --image=<URL or path to file>

    # Apply color splash to video using the last weights you trained
    python3 building_facade.py splash --weights=last --video=<URL or path to file>
"""
...
class CustomConfig(Config):
    ...
    NUM_CLASSES = 1 + 5  # Background + window + wall ...

    # Number of training steps per epoch
    STEPS_PER_EPOCH = 180

    # Skip detections with < 80% confidence
    #NILS: DETECTION_MIN_CONFIDENCE = 0.9
...
class CustomDataset(utils.Dataset):
    ...
    def load_custom(self, dataset_dir, subset):
        ...
        """
        # Add classes.
        self.add_class("custom", 1, "window")
        self.add_class("custom", 2, "wall")
        ...
        for a in annotations:
            ...
            self.add_image(
                "custom",  ## for a single class just add the name here
    ...
    def load_mask(self, image_id):
       ...
       Returns:
        ...
        Index_wall=[] # collect the index of wall class
        Index_all_Other=[]  # Collect the index of other classes
        #count_polygon = len(info["polygons"])
